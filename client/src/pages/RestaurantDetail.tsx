      ...
      <div className="container py-8">
        ...
        {restaurant.galleryImages && restaurant.galleryImages.length > 1 && (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
            {restaurant.galleryImages.slice(1, 7).map((img, i) => (
              <div key={i} className="rounded-lg overflow-hidden aspect-video">
                <img src={img} alt={`${restaurant.name} ${i + 1}`} className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" />
              </div>
            ))}
          </div>
        )}
